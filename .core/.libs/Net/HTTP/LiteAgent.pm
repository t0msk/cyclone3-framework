...
# PREPISAT, PORIADNE ZDOKUMENTOVAT
# A JEDNOTLIVE CASTI PRESUNUT DO INYCH KNIZNIC



package Net::HTTP::LiteAgent;
...
for (0..255)
{
 ...
 #if (!defined($urlencode_valid[$i]))
 #{
  #$urlencode_valid[$i]=sprintf("%%%02X", $i);
 #}
}
...
sub reset
{
  ...
  undef $self->{$_} foreach ("body","request", "content", "status", "proxy","proxyport", "resp-protocol", "error-message","resp-headers", "CBARGS", "callback_function", "callback_params");
...
=cut
  ...
  $self->{HTTPReadBuffer} = "";
  $self->{method} = "GET";
  $self->{headers} = { 'user-agent' => "Tomahawk lib LiteAgent/$VERSION" };
...
# URL-encode data
# TODO: TOTO PRESUNIEM POTOM DO Net::URI::URL::encode
sub escape
{
	my $toencode = shift;
...
sub request
{
  ...
  # Parse URL 
  # TOTO JE UBOHEEEEEEEEEEEEEEEEEEE! VYLEPSIT!!!
  my ($protocol,$host,$junk,$port,$object) = 
    $url =~ m{^([^:/]+)://([^/:]*)(:(\d+))?(/.*)$};

  #print "$url- $protocol,$host,$junk,$port,$object\n";
  
    
  # Only HTTP is supported here
  if ($protocol ne "http")
  {
    warn "Only http is supported";
    return undef;
  }
