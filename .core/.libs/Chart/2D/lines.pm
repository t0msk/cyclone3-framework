...
sub prepare
{
	...
	#
	#print "prepare_axis_calculate()\n";
	$self->prepare_axis_calculate();
	#print "prepare_axis_x_markArea(front=>0)\n";
	...
	foreach my $color(keys %Chart::colors::table)
	{
		...
		$g->stop(offset=>"0%",style=>"stop-color:rgb(".$Chart::colors::table{$color}{'B0'}.");stop-opacity:1");
		#$g->stop(offset=>"50%",style=>"stop-color:rgb(".$Chart::colors::table{$color}{'L1'}.");stop-opacity:1");
		$g->stop(offset=>"100%",style=>"stop-color:rgb(".$Chart::colors::table{$color}{'L1'}.");stop-opacity:1");
	}
	...
	foreach my $column(@{$self->{columnsA}})
	{
		...
		foreach my $row(@{$self->{row}{label}})
		{
			...
			main::_log("draw column='$column' row='$row' data='$data'");
