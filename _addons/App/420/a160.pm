...
sub get_relation_iteminfo
{
	...
	$env{'r_db_name'}=$App::420::db_name unless $env{'r_db_name'};
	
	my $lng_in;
	
	my %info;
	
	if ($env{'r_table'} eq "static")
	{
		my $sql=qq{
			SELECT
				ID,
