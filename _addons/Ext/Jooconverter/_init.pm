...
BEGIN
{
	...
	main::_log("<={jclass} net.sf.jooreports.tools.ConvertDocument");
	main::_log("<={jclass} net.sf.jooreports.openoffice.converter.OpenOfficeDocumentConverter");
	main::_log("<={jclass} net.sf.jooreports.openoffice.connection.OpenOfficeConnection");
	main::_log("<={jclass} net.sf.jooreports.openoffice.connection.SocketOpenOfficeConnection");
	my $DIR=(__FILE__=~/^(.*)\//)[0];
...
use Inline
	...
	STUDY => [ qw(
		java.io.File
		net.sf.jooreports.tools.ConvertDocument
		...
		net.sf.jooreports.openoffice.connection.SocketOpenOfficeConnection
	) ],
	AUTOSTUDY => 1;
...
Extension to java Jooconverter
...
sub connect
{
	...
	$connection=new net::sf::jooreports::openoffice::connection::SocketOpenOfficeConnection($hostname,$port);
	# connect
	eval
	{
		$connection->connect();
...
sub convert
{
	...
	if (!$connection)
	{
		return undef unless Ext::Jooconverter::connect();
	}
