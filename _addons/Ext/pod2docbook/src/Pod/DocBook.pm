...
sub textblock {
	...
	if ($state eq 'title')
	{
		...
		$para_out="<title>".($paragraph)."</title>\n";
	}
	elsif ($state eq 'name')
	{
...
sub _handle_head
{
	...
	# title of document
	if ($parser->{doctype} eq 'article' && $command eq 'head1' && $paragraph eq 'NAME')
	{
		#print $out_fh "<title>".$parser->_indent()."</title>\n";
		push @{$parser->{'Pod::DocBook::state'}}, 'title';
	}
	elsif ($parser->{doctype} eq 'refentry' && $command eq 'head1' && $paragraph eq 'NAME')
	{
