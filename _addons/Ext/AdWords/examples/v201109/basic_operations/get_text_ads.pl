...
sub get_text_ads {
  ...
  # Create predicates.
  my $ad_group_predicate = Google::Ads::AdWords::v201109::Predicate->new({
    field => "AdGroupId",
    operator => "IN",
    values => [$ad_group_id]
  });
  my $status_predicate = Google::Ads::AdWords::v201109::Predicate->new({
    field => "Status",
    operator => "IN",
    values => ["ENABLED", "PAUSED", "DISABLED"]
  });
  my $ad_type_predicate = Google::Ads::AdWords::v201109::Predicate->new({
    field => "AdType",
    operator => "EQUALS",
    values => ["TEXT_AD"]
  });
